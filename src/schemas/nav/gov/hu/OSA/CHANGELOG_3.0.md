...
- The new utilitySettlementIndicator flag has been added to the schema to respond to special rules governing public utility settlement invoices. If the value of the tag is true, amendments with no history (that is, amendments with the modifyWithoutMaster tag set to true) are processed even if the referenced base invoice already exists in the system.
- In an effort to unify automated processing of invoices as much as possible, the conventionalInvoiceInfo node has been added to both the invoice header and the item level. Version 3.0 of the schema supports most frequently used common data, such as PO numbers, delivery note numbers, agreement IDs, G/L account codes, company codes, cost centre codes, etc. in the faith that giving a common and conventional name to such data can help spreading automated processing.
...
- New ERROR: If the value of advancePayment is true, specifying the advancePaymentData node becomes required.
...
- You must specify utilitySettlementIndicator in the invoice header. You have to specify the tag after the exchangeRate tag.
...
- If your software supports public utility settlement invoices, and if you set utilitySettlementIndicator to true in the invoice header, you can submit data exchange regarding an amendment without any history (modifyWithoutMaster=true) even if the referenced base invoice already exists in the system.
